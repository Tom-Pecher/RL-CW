"""
Training script for the A3C agent.
"""
...
def train_agent(hardcore: bool, render: bool):
    ...
    for episode in range(num_episodes):
        ...
        if episode_reward > best_reward:
            ...
            model_dir = "models"
            os.makedirs(os.path.join(model_dir, "a3c"), exist_ok=True)
            torch.save(agent.actor.state_dict(),
                        f"{model_dir}/a3c/best_actor.pth")
            torch.save(agent.critic.state_dict(),
                        f"{model_dir}/a3c/best_critic.pth")
        ...
        if (episode % episode_trigger_count == 0) or (episode == num_episodes - 1):
            ...
            checkpoint_path_actor = f"{model_dir}/a3c/ep_{episode}_actor.pth"
            checkpoint_path_critic = f"{model_dir}/a3c/ep_{episode}_critic.pth"
            ...
            wandb.log({
                "video": wandb.Video(
                    f"{video_dir}/a3c-episode-{episode}.mp4",
                    format="mp4"
                )
            })
