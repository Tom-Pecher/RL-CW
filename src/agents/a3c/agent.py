"""
Agent for the A3C (Advantage Actor Critic) RL.
"""
...
class A3CAgent:
    ...
    def train(self):
        ...
        states, actions, rewards, _, dones = self.memory.sample(
            self.batch_size, self.device
        )

        # Convert to tensors
        states = torch.FloatTensor(np.array(states.cpu())).to(self.device)
        actions = torch.FloatTensor(np.array(actions.cpu())).to(self.device)
